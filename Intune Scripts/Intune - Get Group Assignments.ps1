...
foreach ($m in $mod) {
    if (-not (Get-Module -Name $m -ListAvailable)) {
        Write-Host "Module $m not found. Installing..."
...
$intuneGroupName = Read-Host -Prompt "Enter the name of the group you want to get assignments for"

### Get Azure AD Group
$groupName = "$intuneGroupName"
