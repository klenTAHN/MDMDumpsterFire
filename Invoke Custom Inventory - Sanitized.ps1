...
.NOTES
	...
	2.2 - (2025-27-03) Updated by Clinton Sizemore to include Graph PowerShell API calls for Device user data.
#>
#region initialize
...
####################################################################################################################################################################################################################################################################################
####################################################################################################################################################################################################################################################################################
###Device UserInformation pull 
###Added by Clint Sizemore, 3/11/25
###Functions to get UPN Info for Device

#Check if Microsoft.Graph.DeviceManagement module is installed and install if needed.
$NG = get-packageprovider -ListAvailable -name NuGet -ErrorAction SilentlyContinue
if (!($NG)) {
...
foreach ($module in $MGD) {
    $ModInstalled = Get-Module -ListAvailable -Name $module
	if (!($ModInstalled)) {
...
#Get Device User Name (Last, First)
   function Get-DeviceUserData {
    $TenantID = ""
    $ClientID = ""
	$clientsecretvalue = ""
    ...
    $DUser = Get-MgDeviceManagementManagedDevice -All | Where-Object DeviceName -eq $devicename | select-object -ExpandProperty UserDisplayName
    $DUPN = Get-MgDeviceManagementManagedDevice -All | Where-Object DeviceName -eq $devicename | select-object -ExpandProperty UserPrincipalName
    $DiD = Get-MgUser -UserId $DUPN -Property department | select-object -ExpandProperty department
    $DiID = Get-MgUser -UserId $DUPN | select-object -expandproperty Id
    $DiMiD = Get-MgUserManager -UserId $DiID
    $DiM = Get-MgUser -UserId $DiMiD.Id | select-object -expandproperty DisplayName
    return @{
        User = "$DUser"
        Dept = "$DiD"
        Manager = "$DiM"
...
### Get Device User Information
	$DeviceUser = (Get-DeviceUserData).User
	$UPNDepartment = (Get-DeviceUserData).Dept
	$UPNManager = (Get-DeviceUserData).Manager
...
# Create JSON to Upload to Log Analytics
	...
	$Inventory | Add-Member -MemberType NoteProperty -Name "Primary User" -Value "$DeviceUser" -Force
	$Inventory | Add-Member -MemberType NoteProperty -Name "Department" -Value "$UPNDepartment" -Force
	$Inventory | Add-Member -MemberType NoteProperty -Name "Manager" -Value "$UPNManager" -Force
